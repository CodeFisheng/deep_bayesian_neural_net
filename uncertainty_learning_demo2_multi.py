...
num_epochs = 10000 # training epoches for each customer samples
...
with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
    ...
    for k in range(num_epochs):
        ...
        #     kind = kind + 1
    saver.save(sess, "models/HOME_model_all_10.ckpt")#%HOME_name[10:14])
	"""
    saver.restore(sess, "models/HOME_model_all_50.ckpt")#%HOME_name[10:14])
    ...
    ######################################### resulting for uncertainty modelling
    for k in range(num_realisations):
        if k % num_realisations/10 == 0:
            print 'Ding.....'
        X,Y = train_data_gen()
        reshaped_pred = sess.run([reshaped_outputs_f], feed_dict = {_X_f:X,_Y_f:Y,_Dropout_f: _dropout_train})
        uncertainties.append(reshaped_pred)
        features.append(X)
############################ get marginal samplings
####### data pre-formatting
featurelist = np.array(features).reshape([-1,48])
uncertaintylist = np.array(uncertainties).reshape([-1])
print 'Start Visualization'
#### temperature code: 44  2d histogram this version, the samplings are all following nature prior sampling from the database.
# x list featurelist[:,44]
x = featurelist*data_norm[1:]
y = uncertaintylist*data_norm[0]
#pd.DataFrame(featurelist).to_csv('feature_%s.csv'%HOME_name[10:14], header = None)
#pd.DataFrame(uncertaintylist).to_csv('uncertainty_%s.csv'%HOME_name[10:14], header = None)
pd.DataFrame(x).to_csv('dresult/feature_all_50.csv', header = None)
pd.DataFrame(y).to_csv('dresult/uncertainty_all_50.csv', header = None)
####### uniformed histogram2d from temperature

# pl.pcolor(xedges[5:-6], yedges, pdf(xedges[5:-6], yedges))
# pl.savefig('foo.png')
# xedges
# pd.DataFrame(xcenters).to_csv('ucmmodel_xmatrix.csv', header = None)
# pd.DataFrame(ycenters).to_csv('ucmmodel_ymatrix.csv', header = None)
# pd.DataFrame(H).to_csv('ucmmodel_zmatrix.csv', header = None)
# pl.clf()
# cs = pl.contour(xcenters, ycenters, H, 3, cmap=pl.cm.bone)
# cs = pl.contour(xedges[10:-10], yedges[0:40], pdf(xedges[10:-10], yedges[0:40]), 20, cmap=pl.cm.bone)
# cbar = pl.colorbar(cs)
# pl.savefig('contour.png')
# print 'Formulating pdf'
