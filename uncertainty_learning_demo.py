...
for i in range(ZonalNum):
    tmp = np.array(Zonal[i].data)[0:,:]
    database_f[i] = tmp
database_f = np.transpose(database_f, [1, 0, 2])
totalen_f = rows_f
#print database_f[:,0]
n_input_f = columns_f - 1
database_f[:,:,0] = database_f[:,:,0]/(np.max(database_f[:,:,0]))
#print totalen_f
db_f = database_f
#print db_f
#define id arrays
...
cost_f = tf.reduce_mean(tf.pow(reshaped_results_f - reshaped_outputs_f,2))+0.05*sum(reg_losses)
#cost = tf.nn.l2_loss(reshaped_results-reshaped_outputs)
optimizer_f = tf.train.AdamOptimizer(learning_rate=0.001, beta1 = 0.8, beta2 = 0.7).minimize(cost_f)
...
pinballist = []
#for i in range(50):
#_dropout_train = _dropout_train - 0.01
#print _dropout_train
outlist = []
realist = []
samples = []
samples_res = []
...
with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
	...
	for k in range(num_epochs):
	    ...
	    if (k >= out_thresh) & (k % gap_test_f == 0):
	        ...
	        kind = kind + 1
	saver.save(sess, "tmp_model_%.2f.ckpt"%_dropout_train)
