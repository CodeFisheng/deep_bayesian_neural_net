...
data_norm_min = np.min(database_f, axis = 0)
...
with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
    ...
    #         kind = kind + 1
    # saver.save(sess, "HOME_model_%s.ckpt"%HOME_name[10:14])
    saver.restore(sess, "HOME_model_%s.ckpt"%HOME_name[10:14])
    ...
    maxbound = data_norm[45]
    minbound = data_norm_min[45]
    nbins = 10
    for nbin in range(nbins):
        temp = minbound + (maxbound - minbound)*(0.5+nbin)/6
        for k in range(num_realisations):
            if k % num_realisations/10 == 0:
                print 'Ding.....'
            X,Y = train_data_gen()
            X[:,:,44] = temp
            reshaped_pred, reshaped_feat = sess.run([reshaped_outputs_f, _X_f], feed_dict = {_X_f:X,_Y_f:Y,_Dropout_f: _dropout_train})
            uncertainties.append(reshaped_pred)
            features.append(reshaped_feat)
############################ get marginal samplings
####### data pre-formatting
featurelist = np.array(features).reshape([-1,48])
uncertaintylist = np.array(uncertainties).reshape([-1])
print 'Start Visualization'
#### temperature code: 44  2d histogram
# x list featurelist[:,44]
# y list uncertaintylist
x = featurelist[:,44]*data_norm[45]
...
pl.pcolor(xedges[5:-5], yedges, pdf(xedges[5:-5], yedges))
pl.show()
